...
class GameType(Enum):
    ...
    bug_fables = 328
    civilization_v = 329
    horizon_zero_dawn = 330
    crash_2 = 331
    crash_3 = 332
    spyro_1 = 333
    toy_story_2_version_2 = 334
    pokemon_legends_arceus = 335
    hollow_knight_item_rando_jp = 336
    sos_poot_all_items = 337
    metroid_fusion_rando = 338
    azure_dreams_ps1 = 339
    monkey_island_1_3 = 340
    no_mans_sky = 341
    pokemon_crystal_clear = 342
    disneyland_adventures = 343
    robot_64 = 344
    cod_black_ops_zombies = 345
    gta_v = 346
    lego_city_undercover = 347
    super_mario_rpg_randomizer = 348
    oceanhorn_2 = 349
    animal_crossing_new_horizons = 350
    threads_of_fate = 351
    counter_strike_global_offensive = 352
    hitman_3_paris = 353
    hitman_3_sapienza = 354
    hitman_3_hokkaido = 355
    hitman_3_miami = 356
    hitman_3_santa_fortuna = 357
    hitman_3_whittleton_creek = 358
    hitman_3_berlin = 359
    ...
    crash_1 = auto()
    crash_1_NST = auto()
    bugs_bunny_lost_in_time = auto()
    crash_bash = auto()
    crash_team_racing_2 = auto()
    fallout_3 = auto()
    final_fantasy_5_career_day = auto()
    final_fantasy_7 = auto()
    final_fantasy_7_remake = auto()
    final_fantasy_7_rebirth = auto()
    mario_kart_wii = auto()
    fallout_4 = auto()
    infinite_craft = auto()
    horizon_forbidden_west = auto()
    bluey = auto()
    lil_gator_game = auto()
    pokemon_infinite_fusion = auto()
    mgs_twin_snakes = auto()
    pummel_party = auto()
    demoncrawl = auto()
    wwe_supercard = auto()
    pseudoregalia = auto()
    days_gone = auto()
    harvest_moon_back_to_nature_simple = auto()
    harvest_moon_back_to_nature_tiered = auto()
    pikuniku = auto()
    destiny_2 = auto()
    undertale_genocide = auto()
    splatoon_3 = auto()
    ato = auto()
    kirby_forgotten_land = auto()
    yugioh_tcg = auto()
    the_long_dark = auto()
    hell_pie = auto()
    spyro_reignited = auto()
    king_of_the_castle_multi = auto()
    king_of_the_castle_single = auto()
    peggle_deluxe = auto()
    peggle_nights = auto()
    horizon_zero_dawn_sacred_lands = auto()
    paper_mario_2022 = auto()
    metroid_dread_balanced = auto()
    metroid_dread_chaos = auto()
    terraria_pre_hardmode_journey = auto()
    terraria_hardmode_journey = auto()
    links_awakening_randomizer_dive = auto()
    fnaf_security_breach_everything = auto()
    fnaf_security_breach_no_collectibles = auto()
    fnaf_security_breach_only_collectibles = auto()
    fnaf_security_breach_speedrun_categories = auto()
    hollow_knight_item_rando_blackout = auto()
    elden_ring_randomizer = auto()
    zelda_eow_no_ww = auto()
    dead_rising = auto()
    super_mario_bros_3_randomizer_open_world_mario = auto()
    super_mario_bros_3_randomizer_open_world_race_to_w8 = auto()
    terraria_calamity = auto()
    mul_ty_player = auto()
    sky_children_of_the_light = auto()
    noita = auto()
    super_mario_eclipse = auto()
    elden_ring_eldendawgs = auto()
